...
EbErrorType svt_av1_verify_settings(SequenceControlSet *scs) {
    ...
    if (config->variance_bright_attenuation > 4) {
        SVT_ERROR("Bright attenuation strength must be between 0 and 4\n");
        return_error = EB_ErrorBadParameter;
    }
    ...
    if (config->pass == ENC_FIRST_PASS) {
        ...
        if (config->rate_control_mode != SVT_AV1_RC_MODE_CBR) {
            if (!config->enable_variance_boost) {
                ...
                SVT_INFO(
                    "SVT [config]: AQ mode / VB strength / octile / curve / dark boost / bright atten \t: %d / %d / "
                    "%d / %d / %d / %d\n",
                    config->aq_mode,
