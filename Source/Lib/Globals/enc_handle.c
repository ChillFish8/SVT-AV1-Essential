...
static void set_param_based_on_input(SequenceControlSet *scs)
{
    ...
    if (scs->static_config.variance_dark_boost && !scs->static_config.enable_variance_boost) {
        scs->static_config.variance_dark_boost = 0;
...
static void copy_api_from_app(SequenceControlSet *scs, EbSvtAv1EncConfiguration *config_struct) {
    ...
    scs->static_config.variance_dark_boost = config_struct->variance_dark_boost;
