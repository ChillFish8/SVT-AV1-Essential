...
An additional boost for superblocks that are both dark and low in contrast. Three linear weights are computed per superblock: a luma weight that is 1 at or below an 8-bit mean luma of 64 and falls to 0 at 112,
a contrast weight that is 1 at or below a weighted 8x8 variance of 16 and falls to 0 at 64, and a detail weight that is 0 at or below a busiest-8x8 variance of 0.5 and rises to 1 at 2, which withholds the boost from a solid fill.
The Variance Boost qstep ratio is multiplied by `1 + strength_factor * luma_weight * contrast_weight * detail_weight`, where the strength factor
...
|                                    | 5a. With `--variance-dark-boost` set, the qstep ratio from step 5 is additionally multiplied by a factor that grows for superblocks that are both dark (low mean luma) and low in contrast (low weighted variance), and that falls back to 1 when every 8x8 in the superblock is flat, before clipping. |
